...
//           <button className="btn bg-orange-500 text-black w-full mt-6" onClick={() => setModalOpen(true)}>
//             Request This Food
//           </button>
//         </div>
//       </div>
    ...
    try {
      ...
      if (res.data.modifiedCount > 0) {
        Swal.fire({
          icon: 'success',
          title: 'Request Sent!',
          text: 'Food successfully requested.',
        });
        setModalOpen(false);
      } else {
        Swal.fire({
          ...
          text: 'This food might have already been requested or is unavailable.',
        });
      }
    } catch (error) {
      console.error(error);
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'Failed to request food. Try again later.',
      });
    }
  };
      ...
      {modalOpen && (
        <dialog open className="modal">
          <div className="modal-box max-w-2xl">
            ...
            <div className="modal-action mt-4">
              <button className="btn bg-white text-orange-500 border border-orange-500" onClick={() => setModalOpen(false)}>Cancel</button>
              <button className="btn bg-orange-500 text-white" onClick={handleRequestSubmit}>Request</button>
            </div>
          </div>
        </dialog>
      )}
